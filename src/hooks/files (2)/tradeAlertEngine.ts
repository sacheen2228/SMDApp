...
export function detectTradeIntent(message: string): {
  wantsTrade: boolean;
  kind: "option" | "equity" | "any";
  symbol?: string;
} {
  const text = message.toLowerCase();
  const wantsTrade = /trade|signal|setup|entry|buy|sell|call|put|alert/.test(text);
  if (!wantsTrade) return { wantsTrade: false, kind: "any" };

  const kind: "option" | "equity" | "any" =
    /\b(ce|pe|option|strike|nifty|banknifty|sensex)\b/.test(text) ? "option" :
    /\b(equity|stock|share)\b/.test(text) ? "equity" : "any";

  const symbolMatch = text.match(/\b(nifty|banknifty|sensex|finnifty|midcpnifty|reliance|tcs|infy|hdfcbank|icicibank|sbin)\b/i);
  const symbol = symbolMatch ? symbolMatch[0].toUpperCase() : undefined;

  return { wantsTrade: true, kind, symbol };
}
