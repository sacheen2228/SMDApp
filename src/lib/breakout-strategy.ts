...
// Breakout's UNIQUE input is price-action S/R + fakeout detection; the
// shared engine supplies an additional quality/confidence cross-check so
// the breakout signal inherits the Institutional Engine's read.
  ...
  async simulateFromMarketData(spotPrice: number, vix: number, symbol?: string): Promise<BreakoutSignal | null> {
    ...
    if (symbol) {
      try {
        ...
        candles = (rawCandles.candles || []).map(c => ({
    ...
    if (candles.length > 0) {
      ...
      const sig = this.onTick(lastCandle);
      return sig ? enrichWithEngine(sig, spotPrice, symbol) : sig;
    }
...
// Engine cross-check — blends the Institutional Engine's quality score into
// the breakout signal's confidence so breakout inherits the shared core read.
function enrichWithEngine(
  sig: BreakoutSignal,
  spot: number,
  symbol?: string,
): BreakoutSignal {
  if (!symbol || sig.confidence >= 95) return sig;
  try {
    const expiry = getStandardizedExpiry(symbol);
    const dte = expiry?.days_to_expiry ?? 1;
    const type = sig.direction === 'bullish' ? 'CE' : 'PE';
    ...
    if (engine.best) {
      const blended = Math.round((sig.confidence + engine.best.confidence.score) / 2);
      return { ...sig, confidence: Math.max(sig.confidence, Math.min(95, blended)) };
    }
  } catch {
    // non-fatal — keep original signal
  }
  return sig;
}
