...
//
// Consolidated into the single Institutional Engine strategy family.
// The Master Bot's UNIQUE input is the Yahoo multi-timeframe correlation
// signal; the shared engine supplies the position-quality / confidence
// cross-check via a synthetic equity leg (see engineConfidence()).
...
function calculatePositionSize(
  ...
  // Floor at 0 (not 1): never force a lot that breaches the risk cap.
  return Math.max(Math.floor(maxRisk / riskPerLot), 0);
}
...
// Engine cross-check — runs the shared core on a synthetic delta=1 equity
// leg so the Master Bot inherits the Institutional Engine's quality read.
// Returns 0 when the engine cannot produce a candidate (non-fatal).
function engineConfidence(spot: number, direction: string, atr: number, symbol: string): number {
  try {
    ...
    const req: InstitutionalRequest = {
      ...
      candles: [],
      atr,
      chain: [{ strike: spot, ce: { type: 'CE', ltp: spot, oi: 0, oiChg: 0, volume: 0, iv: 0, delta: 1, gamma: 0, theta: 0, vega: 0, bid: spot, ask: spot }, pe: null }],
      forceType: direction === 'CALL' ? 'CE' : 'PE',
    };
    const engine = evaluateInstitutionalCandidate(req);
    return engine.best ? Math.round(engine.best.confidence.score) : 0;
  } catch {
    return 0;
  }
}
...
export async function generateTradePlan(): Promise<{
  ...
  if (bestDir && agreement >= 2) {
    ...
    if (lots < 1) return { plan: plans, snapshot, signals, raw: data }; // risk budget too small for even 1 lot — skip
    ...
    // Engine cross-check: a synthetic equity leg runs the shared core so the
    // Master Bot's plan inherits the Institutional Engine's quality read.
    const engineConf = engineConfidence(spot, direction, daily?.atr || 50, index);
    if (engineConf > 0) {
      reasons.push(`Engine quality ${engineConf}%`);
    }
    ...
    plans.push({
      ...
      maxRisk,
      rr,
      confidence: engineConf >= 70 ? "HIGH" : agreement >= 3 ? "HIGH" : "MEDIUM",
      timeframe: tfForAtr,
