// ═══════════════════════════════════════════════════════════════════
// ORCA Backtest Engine — Test ORCA signals on historical data
// Generates historical option chains, runs ORCA, tracks outcomes
// ═══════════════════════════════════════════════════════════════════
...
// ─── Types ──────────────────────────────────────────────────────
export interface OrcaBacktestTrade {
  id: number;
  date: string;
  signal: TradeAction;
  strike: number;
...
export interface OrcaDayResult {
  date: string;
  spotOpen: number;
  spotHigh: number;
  spotLow: number;
  ...
  bias: string;
  confidence: number;
  trade: OrcaBacktestTrade | null;
  alerts: string[];
}

export interface OrcaBacktestPerformance {
  totalDays: number;
  tradingDays: number;
  totalSignals: number;
...
export interface OrcaBacktestResult {
  symbol: string;
  startDate: string;
  endDate: string;
  lotSize: number;
  capital: number;
  riskPerTrade: number;
  performance: OrcaBacktestPerformance;
...
function calculatePerformance(
  trades: OrcaBacktestTrade[],
  dailyResults: OrcaDayResult[],
  capital: number
): OrcaBacktestPerformance {
...
// ═══════════════════════════════════════════════════════════════════
// MAIN BACKTEST — Runs ORCA on N historical days
// ═══════════════════════════════════════════════════════════════════
export async function runOrcaBacktest(input: {
  symbol: string;
  days: number;
  capital?: number;
  riskPerTrade?: number;
  ...
  for (let i = 0; i < dates.length; i++) {
    ...
    if (tradeAction === "BUY_CALL" || tradeAction === "BUY_PUT") {
      ...
      // Determine outcome using next day's candles
      const nextDayCandles = i < dates.length - 1
        ? generateDayCandles(symbol, dates[i + 1])
        : [];
