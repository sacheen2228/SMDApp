...
const consensusTextClass = (s: number) =>
  s >= 0.2 ? "text-emerald-400" : s > -0.2 ? "text-muted-foreground" : "text-red-400";

const structureColor = (t: string) =>
  t === "UPTREND" ? "text-emerald-400" : t === "DOWNTREND" ? "text-red-400" : "text-muted-foreground";
    ...
    <div className="rounded-lg border border-border bg-card/80 p-3 space-y-3">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] font-semibold text-foreground uppercase tracking-wide">Multi-Timeframe Analysis</span>
        </div>
      ...
      <div className="space-y-1">
        {timeframes.map((tf) => (
          <div key={tf.tf} className="flex items-center gap-2 px-1.5 py-1 rounded bg-gray-800/50">
            <span className="text-[9px] font-mono font-bold text-foreground w-7">{tf.tf}</span>
      ...
      <div className="flex items-center justify-between pt-1 border-t border-white/5">
        ...
        <span className="text-[8px] text-muted-foreground">{timeframes.length - neutralCount}/{timeframes.length} Trending</span>
      </div>
    </div>
  );
}
